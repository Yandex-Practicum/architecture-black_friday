...
async def root():
    ...
    if topology_type == "Sharded":
        ...
        for shard in shards_list.get("shards", {}):
            shards[shard["_id"]] = {
                "host": shard["host"],
                "replicas_count": len(shard["host"].split("/")[1].split(",")) if "/" in shard["host"] else 1
            }
        
        # Get document count per shard for helloDoc collection
        try:
            pipeline = [{"$collStats": {"storageStats": {}}}]
            collection = db.get_collection("helloDoc")
            stats = await collection.aggregate(pipeline).to_list(100)
            shards_doc_count = {}
            for stat in stats:
                shard_name = stat.get("shard", "unknown")
                doc_count = stat.get("storageStats", {}).get("count", 0)
                shards_doc_count[shard_name] = doc_count
        except Exception:
            shards_doc_count = None
    ...
    return {
        ...
        "shards": shards,
        "shards_doc_count": shards_doc_count,
