...
async def initialize_test_data():
    ...
    if count < 1000:
        ...
        for i in range(count, 1000):
            docs.append({"name": f"test_user_{i}", "value": f"data_{i}"})
        await collection.insert_many(docs)
        logger.info(f"✅ Добавлено {len(docs)} тестовых документов в helloDoc. Всего: 1000")
    else:
        logger.info(f"ℹ️ helloDoc уже содержит {count} документов. Новые не нужны.")
