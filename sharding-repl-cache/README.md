...
Запускаем MongoDB sharded cluster с репликацией, Redis и приложение:
...
Выполняет повторные запросы к эндпоинту `/helloDoc/users` и позволяет проверить, что второй и последующие ответы приходят быстрее благодаря Redis.
