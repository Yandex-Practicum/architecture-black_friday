...
async def startup():
    if REDIS_URL:
        try:
            ...
            # Отключаем кэширование при ошибке
            global cache
            cache = nocache
...
async def list_users(collection_name: str):
    ...
    """
    logger.info(f"Fetching users from collection: {collection_name}")
    time.sleep(1)  # Имитация медленного запроса
    collection = db.get_collection(collection_name)
    result = UserCollection(users=await collection.find().to_list(1000))
    logger.info(f"Found {len(result.users)} users")
    return result
